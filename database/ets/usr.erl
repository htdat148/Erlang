...
-export([add_usr/3, delete_usr/1, set_service/3, set_status/2,
	 delete_distabled/0, lookup_id/1]).
...
%% Exported CLinet functions
%% Operation and maintence API
...
add_usr(PhoneNum, CustId, Plan) when Plan == prepay;
				     Plan == postpay ->
...
set_service(CustId, Service, Flag) when Flag == true;
					Flag == fasle ->
    call({set_service, CustId, Service, Flag}).

set_status(CustId, Status) when Status == disabled;
				Status == enabled ->
    call({set_status, CustId, Status}).

delete_distabled() ->
    call(delete_distabled).
